...
public class DirectMessageRepositoryImpl implements DirectMessageRepositoryCustom{
    ...
    @Override
    public Page<DirectMessage> findQChatRoomAndQSenderOrQReceiver(Pageable pageable, ChatRoom chatRoomId, User user, long cursorId) {
        ...
        List<DirectMessage> directMessages = jpaQueryFactory
                ...
                .fetchJoin()
                .where(directMessage.chatRoom.eq(chatRoomId).and(directMessage.sender.eq(user).or(directMessage.receiver.eq(user))
